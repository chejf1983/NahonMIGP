...
public class MockDev {
    ...
    public MockDev(byte devAddress) {
        ...
        //初始化默认EIA信息
        try {
            this.memory.SetValue(this.memory.EIA, DeviceName.addr, DeviceName.length, NahonConvert.StringToByte("TestNahonDev", DeviceName.length));
            this.memory.SetValue(this.memory.EIA, SoftwareVersion.addr, SoftwareVersion.length, NahonConvert.StringToByte("NS001", SoftwareVersion.length));
            this.memory.SetValue(this.memory.EIA, Hardversion.addr, Hardversion.length, NahonConvert.StringToByte("NH002", Hardversion.length));
        } catch (Exception ex) {
...
//                mem.StartAppResult = true;
                ...
                case IC_ADDR_QEURE_CMD:
                    byte[] tmp = new byte[8];
                    System.arraycopy(NahonConvert.IntegerToByteArray(FlashStartAddress), 0, tmp, 0, 4);
                    System.arraycopy(NahonConvert.IntegerToByteArray(FlashEndAddress), 0, tmp, 4, 4);

                    return new MIGPPacket[]{new MIGPPacket(cmd.srcAddress, this.devAddr, (byte) (cmd.CMD | 0x80), tmp)};
                //擦除falsh
                case FLASH_CLEAN_CMD:
                    int cbnum = NahonConvert.ByteArrayToUShort(cmd.data, 0);
                ...
                //读取FLASH
                case FLASH_READ_CMD:
                    int rbnum = NahonConvert.ByteArrayToUShort(cmd.data, 0);
                ...
                case FLASH_WRITE_CMD:
                    int wbnum = NahonConvert.ByteArrayToUShort(cmd.data, 0);
